...
const GraphStats = {
  ...
  loops(ids, edges, adj) {
    ...
    return {
      ...
      spectralGap: this.spectralGap(ids, adj),
   ...
   *
   * λ₂ of the normalised Laplacian, on the largest connected component. Near
   * zero and there is a cheap cut somewhere; away from zero and every split is
   * expensive, which is what an expander is. It is the axis the Flow modules
   * view is read against — a partition of an expander is an artefact, and
   * without this there is no telling one from a real division.
   *
   ...
   * bisection on Sturm's count. Power iteration was tried first and could not
   * do it: on a graph nearly in two pieces its estimate halved every time the
   * iteration count doubled, so the number it gave was a reading of the
   * iteration count.
   *
   * Mirrors spectral_gap() in gol_spectral.py step for step — the same start
   * vector, the same reorthogonalisation, the same bisection. The parity test
   * compares them.
   */
  spectralGap(ids, adj) {
    ...
    const basis = [vector];
    const alpha = [], beta = [];
    const steps = Math.min(this.SPECTRAL_STEPS, n - 1);
    ...
    for (let step = 0; step < steps; step++) {
      ...
      if (b < this.SPECTRAL_EXHAUSTED) break;
      beta.push(b);
    ...
    return gap === null ? null : Math.min(2, Math.max(0, gap));
  },
  ...
  smallestEigenvalue(alpha, beta) {
    ...
    let lo = 0, hi = 2;
    for (let i = 0; i < this.SPECTRAL_BISECTIONS; i++) {
      const mid = 0.5 * (lo + hi);
  ...
  /** Matching the constants in gol_spectral.py. */
  SPECTRAL_STEPS: 48,
  SPECTRAL_EXHAUSTED: 1e-12,
  SPECTRAL_BISECTIONS: 50,
