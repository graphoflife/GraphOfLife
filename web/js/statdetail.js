...
const StatDetail = {
  ...
  EXPLANATIONS: {
    ...
    rewires: 'Edges an agent handed to another of its own neighbours this phase. The connection (agent, other) becomes (recipient, other), so the agent drops out of the middle and the two it stood between are left joined directly. Only recorded on runs created with rewiring enabled. Where the recipient already knew the far node the two collapse into the one edge a simple graph can hold, so the count is of moves attempted rather than of edges gained.',
