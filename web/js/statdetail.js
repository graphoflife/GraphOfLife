...
const StatDetail = {
  ...
  EXPLANATIONS: {
    ...
    rewires: 'Edges an agent handed to another of its own neighbours this phase. The connection (agent, other) becomes (recipient, other), so the agent drops out of the middle and the two it stood between are left joined directly. Only recorded on runs created with rewiring enabled. This counts the moves that actually landed: one an agent chose but that another agent had already taken the edge out from under is not included. Where the recipient already knew the far node the two collapse into the single edge a simple graph can hold, so the move still counts but the edge total does not rise.',
