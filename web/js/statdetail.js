...
const StatDetail = {
  ...
  EXPLANATIONS: {
    ...
    rewires: 'Edges an agent handed to another of its own neighbours this phase. The connection (agent, other) becomes (recipient, other), so the agent drops out of the middle and the two it stood between are left joined directly. Only recorded on runs created with rewiring enabled. A rewire can never add an edge to the graph: one leaves for every one that arrives, so the total normally holds steady, and where the recipient already knew the far node the arriving edge merges into the one a simple graph can hold and the total falls by one. Both ends of an edge can ask to give that same edge away; it still moves only once, and which claim wins is drawn rather than decided by whichever agent the loop reached first.',
    handovers: 'Connections a parent gave to its newborn this phase: the edge moves from parent to child rather than being copied, so the parent ends up with one fewer. Only recorded on runs created with handover enabled — an absent value means the mechanic was off, which is not the same as it being on and never used. Like a rewire this can never add an edge: normally only the anchor changes and the total holds, but where the newborn was already wired to that neighbour the moved edge merges into the one a simple graph can hold and the total falls by one.',
