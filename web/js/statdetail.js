...
const StatDetail = {
  ...
  EXPLANATIONS: {
    ...
    distinctBrains: 'How many different genomes are present. A brain id names a genotype — a copy keeps its source\'s id and only a mutation makes a new one — so two agents with identical weights count once.',
    ...
    cutRisk: 'The largest share of the population a single edge could cut off. For every bridge, removing it leaves a smaller side and a larger one; this is the biggest smaller side there is, as a fraction of everyone. It matters more here than the bridge count does, because cleanup keeps only the largest connected component: a bridge with a tenth of the world behind it is a tenth-of-the-world extinction, waiting for the zero-flow prune to reach that one edge. A count cannot tell a dumbbell of two halves apart from a blob with three leaves stuck on it — both have bridges, but only one is dangerous. Zero means no bridge costs anything; the ceiling is 50%, which is a graph split down the middle by one connection.',
    cutRiskBefore: 'The same worst-cut share, but measured on the graph as it stood before this phase removed anyone. The recorded frame is the graph after the cull, so the ordinary reading and the cull land in the same row and a cut that severs a whole side moves both at once — which makes cause and effect indistinguishable after the fact. Taken beforehand, it is a state the cull has not touched, so it can be read as a predictor of the cull rather than a consequence of it. Runs made before the engine started recording this simply do not have it.',
    spectralGap: 'How hard the population is to cut in two, as one number: the second-smallest eigenvalue of the normalised Laplacian, on the largest connected group. Near zero means there is a cheap cut somewhere — two halves joined by very little. Away from zero means every way of splitting it is expensive, which is what an expander is, and it is the ceiling of what a graph this sparse can reach. Cheeger\'s inequality ties it to the actual best cut from both sides, so a small value promises a good cut exists and a large one proves none does. It matters most as the axis the Flow modules view is read against: that method finds communities by compressing a random walk and always returns something, so without this there is no telling a real division from a line drawn through an indivisible graph. Read it as an upper bound: on a well-connected graph it is exact, and on one that is nearly in two pieces it is a small number whose smallness is the point rather than its exact size. Found by Lanczos, which is what makes it trustworthy on a crowded spectrum — plain power iteration on these graphs returned a number that halved every time the iteration count doubled, and so was measuring the iteration count.',
    lightningScore: 'How much of this phase\'s token flow goes round in circles. A lightning is a closed loop of flow — tokens leaving an agent, crossing links, and arriving back where they started, like current round a circuit. A loop of L hops carries L tokens, one per edge, and scores L squared, so one token round a triangle is a small thing and ten tokens round a ten-edge loop is a large one. Every token belongs to at most one lightning. This is a LOWER bound and cannot be anything else: maximising the total rewards packing flow into few long loops, which contains the problem of finding the longest cycle, which is Hamiltonian cycle in disguise and has no efficient exact answer. What is reported comes from walking the flow until a node repeats — the circle closes as soon as an agent is in it twice — taking that loop out, and going again, preferring steps to agents not yet in the walk so it finds long loops rather than the first triangle it stumbles into.',
    cyclingShare: 'The share of moved tokens that the lightning search managed to place in a closed loop. More comparable across runs than the raw score, because it does not grow with the size of the world. A lower bound, from the same greedy peel — read it against Flow imbalance, which bounds it from above.',
    lightningLongest: 'The most hops in any single closed loop found this phase. A long lightning means tokens travelling a long way round the graph and returning, which is a much stronger statement about structure than many short ones: a triangle can happen by accident, a forty-hop circuit cannot.',
    flowImbalance: 'The share of flow that conservation forbids from circulating. Every token in a loop leaves its agent and comes back, so a loop moves nobody\'s net balance; whatever imbalance is left over is one-way transport that no decomposition could bend into a circle. Exact, with no heuristic in it, and an upper bound on circulation — the true circulating share can never exceed one minus this. Together with Tokens circulating it brackets the truth: if the two are close, the search has found nearly everything there is; if they are far apart, the score is a weak reading and should be treated as one.',
    netLightningScore: 'The lightning score again, but on the net flow: if one agent sends five tokens to a neighbour and the neighbour sends three back, that is two tokens one way and nothing back. Every link then carries flow in at most one direction, so a two-hop loop cannot exist at all and every lightning left is tokens genuinely going round rather than sloshing between neighbours. This is the stricter claim and the more interesting one — mutual exchange is easy and everywhere, a circuit that survives cancellation is not. On a test case of a ten-hop ring buried in reciprocal noise the gross score is 500 and this is 100, which is the ring and nothing else.',
    netCyclingShare: 'The share of net-flow tokens that ended up in a closed loop. Measured against the net total rather than the gross one, so it answers \'of the tokens that actually went somewhere, how many went round\' instead of being diluted by the reciprocity that netting just removed.',
    netLightningLongest: 'The most hops in any single loop of net flow. The strongest single statement this analysis makes: a long circuit that survives cancelling every reciprocal pair is tokens travelling a long way round the graph and coming back, which cannot happen by accident.',
    netFlowShare: 'How much of the gross token flow survives cancelling reciprocal amounts against each other. Low means neighbours are mostly trading back and forth and little is really going anywhere; high means most flow has a direction. Note that netting cannot change any agent\'s balance, since it removes equal and opposite amounts, so Flow imbalance is the ceiling for the gross and net circulation alike.',
    coreShare: 'The fraction of agents left after repeatedly deleting everyone with a single connection, until nobody has one. What survives is the 2-core: the part of the graph every node of which sits on some loop. What gets peeled away is the fringe — the tree-shaped whiskers hanging off the core. A high share means a solidly interwoven population; a low one means most agents are dangling off a comparatively small centre, and are cheap to lose.',
    ...
    ricciCurvature: 'The Ricci scalar of the graph, from the same outward walk that estimates the dimension. In flat space the shell reached at r steps grows exactly as r to the power d minus one; in curved space that line bends, and the bend is the curvature rather than noise. The shell of a ball in d dimensions with Ricci scalar R goes as r to the (d minus one), times one minus R r squared over 6d, so fitting log shell against both log radius and radius squared gives the dimension from the first coefficient and R from the second. Negative means neighbourhoods hold more than flat space allows, which is what a graph that keeps branching gives — a tree is strongly negative. Positive means they close back on themselves. This also reads as a measure of how expander-like the graph is, since sparse expanders are negatively curved as a theorem. Needs at least four usable radii: three points would fit the three parameters exactly and return a number with no evidence in it, so below that it is left blank.',
    ...
    cladesInWindow: 'How many separate families the population divides into, counting a family as everything descended from one agent alive a few iterations ago. Absent unless every iteration was recorded, since ancestry is a chain and a chain cannot be sampled.',
    distinctParents: 'How many different genomes the living mutated from — one step back, not a count of families. It was called distinct lineages, which it never was: after seventeen iterations of one run there were 105 parent genomes and a single surviving family.',
